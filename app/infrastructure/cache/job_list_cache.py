...
page of results embeds the current version, so bumping the version makes
every previously cached page for that scope unreachable immediately,
without needing to know or delete every limit/offset combination that
was ever cached.
...
def _data_key(scope: str, version: int, limit: int, offset: int) -> str:
    return f"jobs:list:data:{scope}:{version}:{limit}:{offset}"
...
async def get_cached_job_list(
    redis: Redis, scope: str, version: int, limit: int, offset: int
) -> str | None:
    return await redis.get(_data_key(scope, version, limit, offset))


async def set_cached_job_list(
    redis: Redis, scope: str, version: int, limit: int, offset: int, payload: str
) -> None:
    await redis.set(
        _data_key(scope, version, limit, offset),
        payload,
